          ...
          #  mfcc105 =  librosa.feature.mfcc(wave105, sr=16000)
           ...
           # mfccs = [("",mfcc),("09",mfcc09),("095",mfcc095),("105",mfcc105)]
            ...
            for name, mfcc in mfccs:
                ...
                fn = fn.split('.')[0] +  "_" + passage.lower() + "." + fn.split('.')[-1]
                # remove small mfcc files to prevent ctc errors
                print("label: %d, mfcc: %d, Truth value: %r" % (len(label), mfcc.shape[1], (len(label) < mfcc.shape[1])))
                if len(label) < mfcc.shape[1]:
                    # save meta info
                    writer.writerow([fn] + label)
   ...
   # Get trails of speakers
    ...
    for trail in trail_list:
        ...
        for wav_file, label in zip(wav_file_ids,labels):
            ...
            wave = wave[::3]
            # Add Uniform noise
            wave_uniform = wave + np.random.normal(size=wave.shape)
            # Add laplace noise
            wave_laplace = wave + np.random.normal(size = wave.shape)
